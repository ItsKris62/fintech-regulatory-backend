...
| 81 | src/scripts/seed-regulatory-frameworks.ts:5-5 | DATABASE_URL | `const adapter = new PrismaPg({ connectionString: process.env.DATABASE_URL! });` | REFACTOR | Read through typed config; ensure secret never appears in client responses or logs. |
