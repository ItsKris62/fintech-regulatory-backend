...
function createFakePrisma() {
  const rows = new Map<string, { id: string; status: string; callbackUrl: string; callbackError: string | null; callbackDeliveredAt: Date | null; decidedBy: string | null; decidedAt: Date | null }>();
  let counter = 0;

  return {
    rows,
    automationApproval: {
      create: vi.fn().mockImplementation(({ data }: { data: { callbackUrl: string } }) => {
        const id = `appr_${++counter}`;
        rows.set(id, { id, status: 'pending', callbackUrl: data.callbackUrl, callbackError: null, callbackDeliveredAt: null, decidedBy: null, decidedAt: null });
        return Promise.resolve({ id });
      }),
      findUnique: vi.fn().mockImplementation(({ where }: { where: { id: string } }) => Promise.resolve(rows.get(where.id) ?? null)),
      findUniqueOrThrow: vi.fn().mockImplementation(({ where }: { where: { id: string } }) => {
        const row = rows.get(where.id);
        if (!row) throw new Error('not found');
        return Promise.resolve(row);
      }),
      updateMany: vi.fn().mockImplementation(({ where, data }: { where: { id: string; status: string }; data: Partial<{ status: string; decidedBy: string; decidedAt: Date }> }) => {
        const row = rows.get(where.id);
