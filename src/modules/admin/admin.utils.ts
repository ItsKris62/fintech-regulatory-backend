...
export const systemConfigUpdateSchema = z.object({
  ...
  aiApiKey: z.string().trim().max(512).optional(),
  aiDailyCostLimit: z.number().min(0).max(100000).optional(),
  aiPolicyModel: z.string().trim().min(1).max(120).optional(),
  aiQueryModel: z.string().trim().min(1).max(120).optional(),
  aiVerificationModel: z.string().trim().min(1).max(120).optional(),
  aiComplexAnalysisModel: z.string().trim().min(1).max(120).optional(),
  aiPolicyTemperature: z.number().min(0).max(1).optional(),
  aiQueryTemperature: z.number().min(0).max(1).optional(),
  availableAIModels: z.array(z.string().trim().min(1).max(120)).min(1).max(20).optional(),
  ...
  securityAlertEmail: z.string().email().optional(),
});
